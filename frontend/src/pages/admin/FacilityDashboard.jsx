      ...
      if (usersResponse.ok && venuesResponse.ok) {
        ...
        for (const venue of venues) {
          try {
            ...
            if (sportsResponse.ok) {
              ...
              if (venue.isVerified ?? venue.verified) {
              ...
              const isVerified = venue.isVerified ?? venue.verified;
              console.log(
                `Venue ${venue.name} (ID: ${venue.id}) has ${sportsCount} sports/courts - ${isVerified ? "ACTIVE" : "PENDING"}`,
              );
            } else {
        ...
        const facilityOwnersData = facilityOwnerEmails.map((email) => {
          ...
          const ownerActiveSports = ownerVenues
            .filter((venue) => venue.isVerified ?? venue.verified)
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedVenues = allVenues.map((venue) =>
          venue.id === venueId
            ? { ...venue, verified: true, isVerified: true }
            : venue,
        );
        ...
        const _venueCourtCount = verifiedVenue?.sportsCount || 0;
      ...
      <div className={styles.container}>
        ...
        <section className={styles.chartsSection}>
          ...
          <div className={styles.chartCard}>
            ...
            ) : allVenues.filter((v) => !(v.isVerified ?? v.verified)).length >
              0 ? (
              <div className={styles.facilityList}>
                {allVenues
                  .filter((v) => !(v.isVerified ?? v.verified))
                  .map((venue) => (
