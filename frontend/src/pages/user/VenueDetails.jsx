...
// API function to fetch comments for a sport
const fetchSportComments = async (sportId) => {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(`${BASE_URL}/api/comments/sport/${sportId}`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
...
// API function to fetch sport name by ID
const fetchSportDetails = async (sportId) => {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(`${BASE_URL}/api/sports/${sportId}`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
    ...
    return { name: typeof name === "string" ? name : `Sport ${sportId}` };
  } catch (error) {
...
export default function VenueDetails({ userName = "Guest" }) {
  ...
  useEffect(() => {
    const loadVenueDetails = async () => {
      ...
      try {
        ...
        // Load comments (per sport) and compute overall rating locally
        loadComments(venueData);
      } catch (err) {
  ...
  const loadComments = async (venueData) => {
    setLoadingComments(true);
    setCommentsError(null);
    setLoadingRating(true);

    try {
      const sportIds = venueData?.sportIds || [];
      const allComments = [];

      for (const sportId of sportIds) {
        if (sportId == null) continue;
        const c = await fetchSportComments(sportId);
        for (const item of c || []) {
      ...
      // Compute overall rating from comment ratings
      const ratings = allComments
    ...
    return (
      <div className={styles.rating}>
        ...
        <span className={styles.ratingText}>
          {rating.toFixed(1)}{" "}
          {count !== undefined ? `(${count} reviews)` : "(Overall Rating)"}
        </span>
      </div>
    );
  };
                    ...
                    {venue.images.map((image, index) => (
                      <div key={index} className={styles.carouselSlide}>
                        <img
                          ...
                          onError={(e) => {
                            e.target.src =
                              "https://via.placeholder.com/800x400/48A6A7/ffffff?text=Image+Not+Available";
                          }}
                        />
                      </div>
                    ))}
