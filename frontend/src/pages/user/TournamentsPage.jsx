...
export default function TournamentsPage({ userName = "Guest", pageSize = 6 }) {
  ...
  const [_joinedGameIds, setJoinedGameIds] = useState([]);
      ...
      if (response.ok) {
        ...
        const processedTournaments = data.map((tournament) => {
            ...
            if (!tournament.listOfUserEmailJoined?.includes(currentUserEmail)) {
              return {
                ...
                listOfUserEmailJoined: [
                  currentUserEmail,
                  ...(tournament.listOfUserEmailJoined || []),
                ],
              };
            }
          }
          // For all tournaments (including non-owned), ensure owner is in participants list
            ...
            tournament.ownerMail &&
            !tournament.listOfUserEmailJoined?.includes(tournament.ownerMail)
          ) {
            return {
              ...tournament,
              listOfUserEmailJoined: [
        ...
        const othersTournaments = processedTournaments.filter(
          (tournament) =>
  ...
  const fetchUserProfile = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/user-profiles/${userEmail}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const fetchJoinedGameIds = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${BASE_URL}/api/user-profiles/${userEmail}/game-ids`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const fetchGameById = async (gameId) => {
    try {
      ...
      if (response.ok) {
        ...
        if (
          game.ownerMail &&
          !game.listOfUserEmailJoined?.includes(game.ownerMail)
        ) {
          return {
            ...game,
            listOfUserEmailJoined: [
  ...
  const fetchJoinedTournamentsDetails = async () => {
    ...
    try {
      ...
      const ownedTournaments = allTournaments.filter(
        (tournament) => tournament.ownerMail === userEmail,
      );
      ...
      // Calculate stats
      const now = new Date();
      const upcoming = gamesWithOwnership.filter(
        (game) => new Date(game.timeDate) > now,
      );
      const completed = gamesWithOwnership.filter(
        (game) => new Date(game.timeDate) <= now,
      );
      ...
      const favoriteVenue = Object.keys(venueCount).length
        ? Object.keys(venueCount).reduce((a, b) =>
            venueCount[a] > venueCount[b] ? a : b,
          )
        : "N/A";
      const favoriteSport = Object.keys(sportCount).length
        ? Object.keys(sportCount).reduce((a, b) =>
            sportCount[a] > sportCount[b] ? a : b,
          )
        : "N/A";
  ...
  const joinTournament = async (tournamentId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${BASE_URL}/api/games/${tournamentId}/join?userEmail=${encodeURIComponent(userEmail)}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        setMessage({
          type: "error",
          text: errorText || "Failed to join tournament",
        });
        setTimeout(() => setMessage({ type: "", text: "" }), 3000);
        return false;
      }
  ...
  const leaveTournament = async (tournamentId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${BASE_URL}/api/games/${tournamentId}/leave?userEmail=${encodeURIComponent(userEmail)}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        setMessage({
          type: "error",
          text: errorText || "Failed to leave tournament",
        });
        setTimeout(() => setMessage({ type: "", text: "" }), 3000);
        return false;
      }
    ...
    for (const tournament of tournaments) {
      if (
        ...
        try {
          ...
          console.error(
            `Failed to auto-join tournament ${tournament.id}:`,
            error,
          );
          updatedTournaments.push(tournament);
        }
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const processedTournaments =
          await autoJoinOwnedTournaments(tournamentsData);
  ...
  const isTournamentFull = (tournament) => {
    return (
      ...
      (tournament.playersRequired || Infinity)
    );
  };
        ...
        <div className={styles.headerContent}>
          ...
          <nav className={styles.nav}>
            <ul>
              ...
              <li>
                <a href="/dashboard/tournaments" className={styles.active}>
                  Tournaments
                </a>
              </li>
              <li>
          ...
          {activeTab === "available" ? (
            <>
              ...
              {loading ? (
                ...
                <div className={styles.noResults}>
                  ...
                  <p>
                    Check back later for new tournaments or try different search
                    terms.
                  </p>
                </div>
              ) : (
            ...
            <>
              <div className={styles.sectionHeader}>
                ...
                <p>
                  Your tournament statistics, joined games, and owned
                  tournaments
                </p>
              </div>
                    ...
                    {joinedTournaments.length === 0 ? (
                      ...
                      <div className={styles.tournamentsGrid}>
                        {joinedTournaments.map((tournament) => {
                          const isUpcoming =
                            new Date(tournament.timeDate) > new Date();
                          return (
                            <div
                              key={tournament.id}
                              className={`${styles.tournamentCard} ${styles.joinedCard}`}
                            >
                              <div className={styles.cardHeader}>
                                <h3>{tournament.gameName}</h3>
                                <div className={styles.badgeContainer}>
                                  {tournament.isOwned && (
                                    <span
                                      className={`${styles.sportBadge} ${styles.ownerBadge}`}
                                    >
                                      <Trophy size={12} />
                                      Owner
                                    </span>
                                  ...
                                  <span
                                    className={`${styles.sportBadge} ${isUpcoming ? styles.upcoming : styles.completed}`}
                                  >
                              ...
                              <div className={styles.cardBody}>
                                ...
                                <div className={styles.detailRow}>
                                  <Users size={16} />
                                  <span>
                                    {tournament.listOfUserEmailJoined?.length ||
                                      0}
                                    /{tournament.playersRequired} players
                                  </span>
                                </div>
      ...
      <footer className={styles.footer}>
        <div className={styles.footerContent}>
          <div className={styles.footerSection}>
            <h3>QuickCourt</h3>
            <p>
              Your premier destination for sports tournaments and venue booking.
            </p>
          </div>
          <div className={styles.footerSection}>
            <h4>Quick Links</h4>
            <ul>
              <li>
                <a href="/dashboard/venues">Find Venues</a>
              </li>
              <li>
                <a href="/dashboard/tournaments">Join Tournaments</a>
              </li>
              <li>
