...
const getAvailableTimeSlots = (allSlots, selectedDate, bookedSlots) => {
  ...
  return allSlots.filter(
    (slot) => !isTimeSlotBooked(slot.value, selectedDate, bookedSlots),
  );
};
...
const fetchVenueById = async (venueId) => {
  try {
    ...
    return {
      ...
      verified: venue.isVerified ?? venue.verified ?? false,
...
// API function to confirm a RESERVED booking after successful payment
...
// API function to release a RESERVED booking (payment failed/cancelled)
const releaseBookingAPI = async (bookingId) => {
  try {
    const token = localStorage.getItem("token");
    await fetch(`${BASE_URL}/api/bookings/${bookingId}/release`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
...
const fetchBookedSlots = async (venueId, sportId) => {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(
      `${BASE_URL}/api/bookings/slots?venueId=${venueId}&sportId=${sportId}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
...
export default function CourtBooking({ userName = "Guest" }) {
  ...
  const venueId =
    routeVenueId || searchParams.get("venue") || location.state?.venueId;
  ...
  const [pricePerSlot, setPricePerSlot] = useState(0.0);
  const [selectedSportOperatingHours, setSelectedSportOperatingHours] =
    useState("");
  ...
  // Release reservation if user closes tab or navigates away
  useEffect(() => {
    const handleBeforeUnload = () => {
      if (pendingBookingData?.bookingId) {
        // Use sendBeacon for reliable fire-and-forget on tab close
        navigator.sendBeacon(
          `${BASE_URL}/api/bookings/${pendingBookingData.bookingId}/release`,
    ...
    const loadVenueDetails = async () => {
      ...
      try {
        ...
        if (sportsData.length > 0) {
          ...
          const sportTimeSlots = generateSportTimeSlots(
            firstSport.operatingHours,
          );
          ...
          if (firstSport.operatingHours) {
            setTimeValidationMessage(
              `${firstSport.name} is available from ${firstSport.operatingHours}`,
            );
          }
        }
      } catch (err) {
  ...
  const handleSportChange = (e) => {
    ...
    if (selectedSportData) {
      ...
      const sportTimeSlots = generateSportTimeSlots(
        selectedSportData.operatingHours,
      );
      setAvailableTimeSlots(sportTimeSlots);
      ...
      if (selectedSportData.operatingHours) {
        setTimeValidationMessage(
          `${sportName} is available from ${selectedSportData.operatingHours}`,
        );
      }
  ...
  const handleStartTimeChange = (e) => {
    ...
    if (isTimeSlotBooked(time, selectedDate, bookedSlots)) {
      setTimeValidationMessage(
        `This sport is reserved by someone else at this time. Please choose a different time or date.`,
      );
      return;
    }
    ...
    if (
      selectedSportOperatingHours &&
      !isTimeWithinOperatingHours(time, selectedSportOperatingHours)
    ) {
      setTimeValidationMessage(
        `Selected time is outside operating hours. ${selectedSport} is available from ${selectedSportOperatingHours}`,
      );
    } else {
      setTimeValidationMessage(
        selectedSportOperatingHours
          ? `${selectedSport} is available from ${selectedSportOperatingHours}`
          : "",
      );
    }
  };
  ...
  const handleBookingSubmit = async (e) => {
    ...
    if (selectedSportOperatingHours) {
      if (!isTimeWithinOperatingHours(startTime, selectedSportOperatingHours)) {
        alert(
          `Start time is outside operating hours. ${selectedSport} is available from ${selectedSportOperatingHours}`,
        );
        return;
      }
      ...
      if (!isTimeWithinOperatingHours(endTime, selectedSportOperatingHours)) {
        alert(
          `End time is outside operating hours. ${selectedSport} is available from ${selectedSportOperatingHours}`,
        );
        return;
      }
    }
    ...
    if (isTimeSlotBooked(startTime, selectedDate, bookedSlots)) {
      alert(
        `Start time ${formatTime(startTime)} is already booked. Please choose a different time.`,
      );
      return;
    }
    ...
    if (isTimeSlotBooked(endTime, selectedDate, bookedSlots)) {
      alert(
        `End time ${formatTime(endTime)} conflicts with existing booking. Please choose a different time.`,
      );
      return;
    }
    ...
    if (hasConflict) {
      alert(
        `Selected time slot conflicts with existing bookings. Please choose a different time.`,
      );
      return;
    }
    ...
    // First, attempt to create the booking BEFORE payment
    try {
      setBookingLoading(true);
      const result = await createBooking(bookingData);
      console.log("Booking reserved:", result);

      // Booking succeeded — now show payment modal
      setPendingBookingData({ ...bookingData, bookingId: result.id });
      setShowPaymentModal(true);
    } catch (err) {
      if (err.status === 409) {
        alert(
          "⚠️ This time slot was just booked by someone else. Please choose a different time.",
        );
        // Refresh booked slots so the UI reflects the new state
        loadBookedSlots();
      } else {
  ...
  const handlePaymentSuccess = async (paymentResult) => {
    ...
    try {
      // Confirm the RESERVED booking on the backend
      await confirmBookingAPI(pendingBookingData.bookingId);

      setSuccessMessage(
        `Payment successful! Booking confirmed. Payment ID: ${paymentResult.paymentIntent.id}`,
      );
  ...
  const handlePaymentError = (errorMessage) => {
    ...
    if (pendingBookingData?.bookingId) {
      releaseBookingAPI(pendingBookingData.bookingId);
      setPendingBookingData(null);
      setShowPaymentModal(false);
      loadBookedSlots(); // Refresh available slots
    }
  };

  const handlePaymentCancel = () => {
    // Release the reservation since user cancelled
    if (pendingBookingData?.bookingId) {
      releaseBookingAPI(pendingBookingData.bookingId);
    }
    setShowPaymentModal(false);
    ...
    return (
      <div className={styles.errorContainer}>
        <div className={styles.errorContent}>
          ...
          <p>
            {error || "The venue you are trying to book could not be found."}
          </p>
            ...
            <div className={styles.venueCard}>
              {venue.images.length > 0 && (
                <div className={styles.venueImageContainer}>
                  <img
                    ...
                    onError={(e) => {
                      e.target.src =
                        "https://via.placeholder.com/400x200/48A6A7/ffffff?text=No+Image";
                    }}
                  />
                </div>
              )}
              ...
              <div className={styles.venueInfo}>
                ...
                <p className={styles.venueDescription}>
                  {venue.description ||
                    "Premium sports facility with modern amenities."}
                </p>
                    ...
                    >
                      ...
                      {sports.map((sport) => (
                        <option key={sport.id} value={sport.name}>
                          {sport.name} - {formatPrice(sport.pricePerHour)}/hour
                          ({sport.type})
                        </option>
                      ))}
                    </select>
                ...
                <div className={styles.timeRow}>
                  <div className={styles.formGroup}>
                    ...
                    >
                      ...
                      {(selectedSportOperatingHours
                        ? generateSportTimeSlots(selectedSportOperatingHours)
