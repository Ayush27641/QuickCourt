...
export default function ProfilePage({
  ...
  const [_reviewMessage, setReviewMessage] = useState({ type: "", text: "" });
  ...
  const [_isLoadingBookings, setIsLoadingBookings] = useState(true);
  const [_bookingsError, setBookingsError] = useState("");
  ...
  const [loadingJoinedTournaments, setLoadingJoinedTournaments] =
    useState(true);
  const [_userGameProfile, setUserGameProfile] = useState(null);
  ...
  const _showingStart = startIndex + 1;
  const _showingEnd = Math.min(startIndex + pageSize, enrichedBookings.length);
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/data/${encodeURIComponent(userEmail)}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className={`${styles.page} ${isVisible ? styles.fadeIn : ""}`}>
        ...
        <div className={styles.layout}>
          ...
          <aside className={styles.sidebar}>
            ...
            <nav className={styles.navigation}>
              ...
              <button
                className={`${styles.navBtn} ${activeNav === "tournaments" ? styles.navActive : ""}`}
          ...
          <main className={styles.content}>
                          ...
                          refundStatus[booking.id] && (
                            <div className={styles.refundStatus}>
                              <div className={styles.refundInfo}>
                                ...
                                <span
                                  className={`${styles.refundStatusBadge} ${styles[refundStatus[booking.id].status.toLowerCase()]}`}
                                >
            ...
            {activeNav === "profile" && (
              <div className={styles.profileSection}>
                ...
                {updateMessage.text && (
                  <div
                    className={`${styles.updateMessage} ${styles[updateMessage.type]}`}
                  >
      ...
      {showLogoutToast && (
        <div
          className={`${styles.logoutToastBackdrop} ${showLogoutToast ? styles.visible : ""}`}
          onClick={handleLogoutCancel}
        >
          <div
            className={`${styles.logoutToast} ${showLogoutToast ? styles.visible : ""}`}
            ...
            <form onSubmit={handleTournamentSubmit}>
              <div className={styles.modalBody}>
                ...
                {tournamentMessage.text && (
                  <div
                    className={`${styles.message} ${styles[tournamentMessage.type]}`}
                  >
                    {tournamentMessage.text}
                  </div>
                )}
